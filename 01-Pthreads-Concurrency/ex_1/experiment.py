...
par_time_pattern = re.compile(r"Total parallel multiplication time:\s*([\d.]+)\s*s")
...
def get_mean_time(mode, degree, threads=None):
    ...
    for run in range(num_runs):
        cmd = [executable, "-d", str(degree), "-m", mode, "--time", "--algorithm", "karatsuba"]
        if threads:
            cmd += ["-t", str(threads)]
...
plot_parallel_efficiency(results)